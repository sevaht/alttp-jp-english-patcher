...
def main():
    ...
    #    colors 1-7 each). Each is a plain slice of the US ROM's PaletteData table; generate.py's
    #    overlay writes them onto the $7EC300/$7EC500 CGRAM buffers. Rows 5/7 are US-specific
    #    palette DATA (row 7 is the wood name-banner); rows 9/11 hold identical ROM bytes JP<->US
    #    but the ported US file-select indexes a different palette there. (Offsets are PaletteData
    #    entries in bank $1B: $1BD9AA, PaletteData_owanim_00 $1BE604, PaletteData $1BD218, $1BD254.)
